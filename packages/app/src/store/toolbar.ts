...
const { EventHost, ToolHost } = require('@editor/core');
...
export const useToolbarStore = defineStore('Toolbar', () => {
  ...
  disposers.add(
    EventHost.on('@app/tools:prepare', (item) => {
      toolsMap.set(item.id, item);
    }),
  );

  disposers.add(
    EventHost.on('@app/tools:remove', (id) => {
      toolsMap.delete(id);
    }),
  );

  disposers.add(
    EventHost.on('@app/tools:activate', (id) => {
