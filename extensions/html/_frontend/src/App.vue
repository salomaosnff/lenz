...
import { getData } from "lenz:ui";
import CssNumberInput from "./components/CssNumberInput.vue";

const { result, selection, parentElement } = getData();
...
selection.addListener(updateSelection);
parentElement.addListener(updateParent);
...
const prettyGridTemplateAreas = computed({
  get() {
    ...
    return form.value["grid-template-areas"].replace(/"\s+"/gmi, '"\n"');
  },
  set(value: string) {
